...
export default function Routine() {
  ...
  const [routineInput, setRoutineInput] = useState("");
    ...
    const handleSubmit = (e) => {
      ...
      return (
          <section id="routine" className="justify-self-end col-start-3 text-left">
...
{updateRoutine && <button className="ml-2" onClick={() => handleDelete(index)}>
          ...
          <form onSubmit={handleSubmit}>
            <button className="text-center bg-[#c8839a] rounded-md border-none px-2 text-xs h-7" type="button" onClick={() => setUpdateRoutine(!updateRoutine)}>{!updateRoutine ? "Update routine" : "Finished updating"}</button>
             {updateRoutine && 
             <div>
             <input
